...
unsigned char probe_angle_char[2];
unsigned char crc_char[4];
unsigned char adc_char[2*BUFF_SIZE];
unsigned char crc_input[4+1+2+2+2*BUFF_SIZE];
unsigned char message_buff[10+4+1+2+2+2*BUFF_SIZE+4];
...
int main(int argc, char **arg){
    /* piezo servo setup */
    int servo_resolution = 200;
    float servo_angles[servo_resolution];
    float servo_start = 95-25;
    float servo_end = 95+25;
    ...
    /* lx-16a servo setup */
    int lx16_resolution = 180;
    float lx16_angles[lx16_resolution];
    float lx16_start = 150-90;
    float lx16_end = 150+90;
    ...
    for (int n = 0; n < lx16_resolution; n++){
        ...
        for (int j = 0; j < servo_resolution; j++){
            ...
            /* ##################### Servo ##################### */
            // read encoder
            int angle_index = j%servo_resolution;
            float servo_angle = servo_angles[angle_index];
            printf("Angle is %f\n", servo_angle);
            control_servo(servo_angle);
            servo_angle = servo_angle * 4096.0 / 360.0;
            encoder = (int)servo_angle;
            // convert small endian to big endian
            encoder = changed_endian_2Bytes(encoder);
            // put number in bytes
            memcpy(encoder_char, &encoder, sizeof encoder_char);
            /* ##################### LX-16A ##################### */
            probe_angle = (int)(lx16_angles[n] * 4096.0/360.0);
            probe_angle = changed_endian_2Bytes(probe_angle);
            memcpy(probe_angle_char, &probe_angle, sizeof probe_angle_char);
            ...
            memcpy(crc_input+sizeof(time_stamp_char)+sizeof(information_byte)+sizeof(encoder_char), probe_angle_char, sizeof(probe_angle_char));
            memcpy(crc_input+sizeof(time_stamp_char)+sizeof(information_byte)+sizeof(encoder_char)+sizeof(probe_angle_char), adc_char, sizeof(adc_char));
